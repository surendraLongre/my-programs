...
input:
	...
	void whoWon() {
		for(int i=0; i!=3; i++) {
			if(arr[i][0]==arr[i][1] && arr[i][1]==arr[i][2]) gameEnd();
			if(arr[0][i]==arr[1][i] && arr[1][i]==arr[2][i]) gameEnd();
		}
		if(arr[0][0]==arr[1][1] && arr[1][1]==arr[2][2]) gameEnd();
		if(arr[0][2]==arr[1][1] && arr[1][1]==arr[2][0]) gameEnd();
	}
